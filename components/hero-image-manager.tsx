...
export function HeroImageManager() {
  ...
  const handleDelete = async (imageId: number | string) => {
    if (confirm('Are you sure you want to delete this hero image?')) {
      try {
        const success = await deleteHeroImage(imageId)
        if (success) {
          toast({
            title: "Success!",
  ...
  const saveEdit = async () => {
    if (editingId) {
      try {
        const success = await updateHeroImage(editingId, editForm)
        if (success) {
          setEditingId(null)
          setEditForm({})
          toast({
  ...
  const handleRefresh = async () => {
    try {
      ...
      toast({
        title: "Refreshed!",
        description: `Loaded ${heroImages.length} hero images`,
      })
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to refresh hero images",
        variant: "destructive",
      })
    }
  }
    ...
    try {
      ...
      if (image.is_active) {
        ...
        if (success) {
          toast({
            title: "Success!",
            description: `Content visibility ${newShowContent ? 'enabled' : 'disabled'}`,
          })
        }
      } else {
        ...
        if (success) {
          toast({
            title: "Success!",
            description: "Hero image activated successfully",
          })
        }
      }
    } catch (error) {
